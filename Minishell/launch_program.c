...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/09/30 18:52:55 by lchant            #+#    #+#             */
/*   Updated: 2017/09/30 18:52:56 by lchant           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		try_launch_path(t_command *command)
{
	...
	if (path != NULL)
	{
		while (path_list[i])
		{
			...
			if ((execve(path_list[i++], command_to_array(command),
			env_list_to_arr(g_shell.env_list)) != -1))
				found = 1;
		}
	}
	if (!found)
...
{
	if (prev == NULL && command->next == NULL)
	{
	...
	return (is_builtin(command, prev));
}
...
int		launch_program(t_command *command)
{
	...
	{
		execve(command->file_name, command_to_array(command),
		env_list_to_arr(g_shell.env_list));
